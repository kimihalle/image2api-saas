...
type EventListFilter struct {
	...
	Provider      string   // when set, keep ONLY rows handled by this provider
...
type StaleEvent struct {
	...
	CreditTransactionID string  `gorm:"column:credit_transaction_id"`
}
...
func (r *EventRepository) PurgeStale(ctx context.Context, maxAge time.Duration) ([]StaleEvent, error) {
	...
	cutoff := time.Now().Add(-maxAge)
	videoCutoff := time.Now().Add(-35 * time.Minute)
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&model.EventLog{}).
			Where("status = ? AND ((kind <> ? AND ts < ?) OR (kind = ? AND updated_at < ?))", "pending", "video", cutoff, "video", videoCutoff).
			Select("id", "user_id", "account_id", "cost", "credit_transaction_id").
...
func (r *EventRepository) UpdateAsyncVideo(ctx context.Context, eventID, upstreamID string, progress int, upstreamCost float64, requestMeta map[string]any, nextPollAt *time.Time) error {
	patch := map[string]any{"updated_at": time.Now()}
	if upstreamID != "" {
		patch["upstream_task_id"] = upstreamID
	}
	if progress >= 0 {
		patch["progress"] = progress
	}
	if upstreamCost > 0 {
		patch["upstream_cost"] = upstreamCost
	}
	if requestMeta != nil {
	...
	if nextPollAt != nil {
		...
		patch["poll_attempts"] = gorm.Expr("poll_attempts + 1")
	}
	return r.db.WithContext(ctx).Model(&model.EventLog{}).Where("id = ?", eventID).Updates(patch).Error
}

func (r *EventRepository) PendingProviderVideos(ctx context.Context, provider string) ([]model.EventLog, error) {
	var items []model.EventLog
	err := r.db.WithContext(ctx).Where("kind = ? AND status = ? AND provider = ? AND upstream_task_id <> ''", "video", "pending", provider).Order("ts asc").Find(&items).Error
	return items, err
}
...
func (r *EventRepository) SetCreditTransactionID(ctx context.Context, eventID, transactionID string) error {
	return r.db.WithContext(ctx).Model(&model.EventLog{}).
		Where("id = ?", eventID).
		Update("credit_transaction_id", transactionID).Error
}
...
// DeleteOwnedByID removes one completed generation record after the caller's
// ownership has already been verified by the service layer.
func (r *EventRepository) DeleteOwnedByID(ctx context.Context, id, userID string) error {
	result := r.db.WithContext(ctx).
		Where("id = ? AND user_id = ?", strings.TrimSpace(id), strings.TrimSpace(userID)).
		Delete(&model.EventLog{})
	if result.Error != nil {
		return result.Error
	}
