...
func NewPaymentHandler(pay *service.PaymentService, publicOrigin string) *PaymentHandler {
	return &PaymentHandler{pay: pay, publicOrigin: strings.TrimRight(strings.TrimSpace(publicOrigin), "/")}
}

func (h *PaymentHandler) notifyBase(c *gin.Context) string {
	if h.publicOrigin != "" {
...
func (h *PaymentHandler) Recharge(c *gin.Context) {
	...
	base := h.notifyBase(c)
	order, err := h.pay.CreateOrder(c.Request.Context(), user, body.Amount, body.Method, body.PackageID, base, clientIP(c))
	if err != nil {
		h.writeErr(c, err)
		return
	}
...
func (h *PaymentHandler) Notify(c *gin.Context) {
	params, err := paymentNotifyParams(c.Request)
	if err != nil {
...
func paymentNotifyParams(r *http.Request) (map[string]string, error) {
	if err := r.ParseForm(); err != nil {
		return nil, err
	}
	params := make(map[string]string, len(r.Form))
	// Request.Form merges query parameters and form-encoded POST fields.
...
func (h *PaymentHandler) writeErr(c *gin.Context, err error) {
	...
	case errors.Is(err, service.ErrPayAmount):
		minimum := h.pay.Settings(c.Request.Context()).MinAmount
		if minimum <= 0 {
			minimum = 0.01
		}
		c.JSON(http.StatusBadRequest, gin.H{"detail": fmt.Sprintf("最低充值金额为 ¥%.2f", minimum)})
