...
func (h *AppSettingsHandler) SMTPTemplatesGet(c *gin.Context) {
	data, err := h.settings.SMTPTemplates(c.Request.Context())
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"detail": "failed to load smtp templates"})
		return
	}
	c.JSON(http.StatusOK, data)
}

func (h *AppSettingsHandler) SMTPTemplatesPut(c *gin.Context) {
	var body service.SMTPTemplateSettings
	if err := c.ShouldBindJSON(&body); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"detail": "invalid request body"})
		return
	}
	data, err := h.settings.SaveSMTPTemplates(c.Request.Context(), body)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"detail": err.Error()})
		return
	}
...
func (h *AppSettingsHandler) ProxyPut(c *gin.Context) {
	var body service.ProxySettings
	if err := c.ShouldBindJSON(&body); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"detail": "invalid request body"})
		return
	}
	data, err := h.settings.SaveProxy(c.Request.Context(), body)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"detail": err.Error()})
		return
	}
