...
// are performed by GenerationQueueService workers outside the HTTP lifecycle.
func (h *UserGenerationHandler) SubmitJobs(c *gin.Context) {
	user := currentUser(c)
	if user == nil {
	...
	items, err := h.queue.SubmitImages(c.Request.Context(), user, service.UserGenerateRequest{
		Model:           body.Model,
		Prompt:          body.Prompt,
		Ratio:           body.Ratio,
	...
	c.JSON(http.StatusAccepted, gin.H{"data": items})
}

func (h *UserGenerationHandler) ActiveJobs(c *gin.Context) {
	user := currentUser(c)
	if user == nil {
		c.JSON(http.StatusUnauthorized, gin.H{"detail": "未登录或会话已过期"})
		return
	}
	items, err := h.queue.ActiveOwned(c.Request.Context(), user.ID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"detail": "读取任务队列失败"})
		return
	}
...
func (h *UserGenerationHandler) Job(c *gin.Context) {
	...
	item, err := h.queue.GetOwned(c.Request.Context(), user.ID, c.Param("id"))
	if err != nil {
...
func (h *UserGenerationHandler) CancelJob(c *gin.Context) {
	user := currentUser(c)
	if user == nil {
		c.JSON(http.StatusUnauthorized, gin.H{"detail": "未登录或会话已过期"})
		return
	}
	if err := h.queue.CancelOwned(c.Request.Context(), user.ID, c.Param("id")); err != nil {
...
func (h *UserGenerationHandler) DeleteImageHistory(c *gin.Context) {
	user := currentUser(c)
	if user == nil {
		c.JSON(http.StatusUnauthorized, gin.H{"detail": "未登录或会话已过期"})
		return
	}
	if err := h.userGen.DeleteImageHistory(c.Request.Context(), user, c.Param("id")); err != nil {
...
func writeQueueError(c *gin.Context, err error) {
	...
	case errors.Is(err, service.ErrUnknownModel):
		c.JSON(http.StatusNotFound, gin.H{"detail": "模型已停用或不存在"})
	case errors.Is(err, service.ErrUnsupportedParams), errors.Is(err, service.ErrBannedPrompt):
		c.JSON(http.StatusBadRequest, gin.H{"detail": err.Error()})
	case errors.Is(err, service.ErrInsufficientFunds):
		c.JSON(http.StatusPaymentRequired, gin.H{"detail": "可用额度不足"})
	default:
		c.JSON(http.StatusBadRequest, gin.H{"detail": err.Error()})
	}
}
...
func (h *UserGenerationHandler) catalogEntries(c *gin.Context) ([]gin.H, error) {
	items := []gin.H{
		...
		},
		{
			...
			"description":          "Adobe Flux Kontext Max (文生图 / 图生图)",
		},
		{
		...
		},
		{
			...
			"max_reference_images": 9,
			"reference_mode":       "frame",
			"description":          "Veo 3.1 Fast (兼容模型 ID)",
		},
		{
		...
		},
		{
			...
			"max_reference_images": 3,
			"reference_mode":       "asset",
			"description":          "Grok Imagine Quality (文生图 / 图生图)",
		},
		{
			"id":                   "seedream-4.5",
			"provider":             "leonardo",
...
func (h *UserGenerationHandler) publicModels() ([]gin.H, error) {
	items := []gin.H{
		...
		},
		{
			...
			"ratios":      []string{"1:1", "2:3", "3:2", "3:4", "4:3", "9:16", "16:9"},
			"resolutions": []string{"1K", "2K"},
			"description": "Grok Imagine Quality",
			"stub":        false,
		},
		{
			"id":          "seedream-4.5",
