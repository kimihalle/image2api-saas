...
type CreditSettings struct {
	RegistrationReward int  `json:"registration_reward"`
...
type ProxySettings struct {
	...
	VideoProxy string `json:"video_proxy"`
}
...
func (s *AppSettingsService) SMTPTemplates(ctx context.Context) (*SMTPTemplateSettings, error) {
	title, err := s.settings.GetValue(ctx, "site.title")
	if err != nil {
		return nil, err
	}
	data := loadSMTPTemplateSettings(ctx, s.settings, title)
	return &data, nil
}

func (s *AppSettingsService) SaveSMTPTemplates(ctx context.Context, in SMTPTemplateSettings) (*SMTPTemplateSettings, error) {
	clean := func(t EmailTemplate, required bool) (EmailTemplate, error) {
		t.Subject = strings.TrimSpace(strings.ReplaceAll(strings.ReplaceAll(t.Subject, "\r", ""), "\n", " "))
		t.HTML = strings.TrimSpace(t.HTML)
		if required && (t.Subject == "" || t.HTML == "") {
			return t, errors.New("验证码邮件的主题和 HTML 内容不能为空")
		}
		if len(t.Subject) > 240 {
			return t, errors.New("邮件主题不能超过 240 个字符")
		}
		if len(t.HTML) > 100000 {
			return t, errors.New("单个邮件模板不能超过 100KB")
		}
		return t, nil
	}
	var err error
	if in.Register, err = clean(in.Register, true); err != nil {
		return nil, err
	}
	if in.Reset, err = clean(in.Reset, true); err != nil {
		return nil, err
	}
	if in.Welcome, err = clean(in.Welcome, in.WelcomeEnabled); err != nil {
		return nil, err
	}
	if err := s.settings.UpsertValues(ctx, map[string]string{
		"smtp.template.register.subject": in.Register.Subject,
		"smtp.template.register.html":    in.Register.HTML,
		"smtp.template.reset.subject":    in.Reset.Subject,
		"smtp.template.reset.html":       in.Reset.HTML,
		"smtp.template.welcome.subject":  in.Welcome.Subject,
		"smtp.template.welcome.html":     in.Welcome.HTML,
		"smtp.template.welcome.enabled":  strconv.FormatBool(in.WelcomeEnabled),
	}); err != nil {
		return nil, err
	}
	return s.SMTPTemplates(ctx)
}
...
func (s *AppSettingsService) Proxy(ctx context.Context) (*ProxySettings, error) {
	...
	imageProxy, err := s.settings.GetValue(ctx, "proxy.image_url")
	if err != nil {
		return nil, err
	}
	videoProxy, err := s.settings.GetValue(ctx, "proxy.video_url")
	if err != nil {
		return nil, err
	}
	return &ProxySettings{
		Proxy:      strings.TrimSpace(proxy),
		ImageProxy: strings.TrimSpace(imageProxy),
		VideoProxy: strings.TrimSpace(videoProxy),
	}, nil
}

func (s *AppSettingsService) SaveProxy(ctx context.Context, in ProxySettings) (*ProxySettings, error) {
	if err := s.settings.UpsertValues(ctx, map[string]string{
		"proxy.url":       strings.TrimSpace(in.Proxy),
		"proxy.image_url": strings.TrimSpace(in.ImageProxy),
		"proxy.video_url": strings.TrimSpace(in.VideoProxy),
	}); err != nil {
		return nil, err
	}
	return s.Proxy(ctx)
...
func (s *AppSettingsService) Credits(ctx context.Context) (*CreditSettings, error) {
	registrationRewardRaw, err := s.settings.GetValue(ctx, "credits.registration_reward")
	if err != nil {
		return nil, err
	}
	...
	return &CreditSettings{
		RegistrationReward: clampNonNegative(parseIntSetting(registrationRewardRaw, 0)),
...
func (s *AppSettingsService) SaveCredits(ctx context.Context, in CreditSettings) (*CreditSettings, error) {
	...
	if in.RegistrationReward < 0 {
		in.RegistrationReward = 0
	}
	if err := s.settings.UpsertValues(ctx, map[string]string{
		"credits.registration_reward": strconv.Itoa(in.RegistrationReward),
...
func (s *AppSettingsService) loadSMTPConfig(ctx context.Context) (SMTPConfig, error) {
	...
	ttlRaw, _ := s.settings.GetValue(ctx, "auth.code_ttl_seconds")
	ttl := parseIntSetting(ttlRaw, 600)
	return SMTPConfig{
		Host:          current.Host,
		Port:          current.Port,
		...
		SiteName:      title,
		ExpireMinutes: ttl / 60,
		Templates:     loadSMTPTemplateSettings(ctx, s.settings, title),
	}, nil
}
...
func (s *AppSettingsService) pruneGeneratedFiles(ctx context.Context, maxAge time.Duration) (int, int64) {
	...
	for _, o := range objs {
		...
		if strings.HasPrefix(key, "branding/") {
			continue // never expire site branding or homepage assets as user works
		}
		if !o.LastModified.Before(cutoff) {
			continue
		}
		if isReferenceFile(key) || IsThumbKey(key) || IsLastFrameKey(key) || mediaKind(key) == "" {
			continue
		}
