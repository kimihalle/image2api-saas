...
type AuthService struct {
	...
	allowFirstAdminRegistration bool
}
...
func NewAuthService(
	...
	return &AuthService{
		...
		cgroups:                     cgroups,
		allowFirstAdminRegistration: allowFirstAdminRegistration,
	}
}
...
func (s *AuthService) SendCode(ctx context.Context, email, purpose string) error {
	...
	if err := s.smtp.SendCode(ctx, s.loadSMTPSettings(ctx), normalizedEmail, code, purpose); err != nil {
		_ = s.codes.Revoke(ctx, normalizedEmail, purpose)
		return err
	}
	return nil
}
...
func (s *AuthService) Register(ctx context.Context, email, username, password, inviteCode, emailCode, ip string) (*model.User, string, *SessionPayload, error) {
	...
	if !hasAdmin && !s.allowFirstAdminRegistration {
		return nil, "", nil, errors.New("系统管理员尚未初始化")
	}
	...
	creditSettings, err := s.loadCreditSettings(ctx)
	if err != nil {
		return nil, "", nil, err
	}
	...
	user := &model.User{
		...
		Role:         role,
		Status:       "active",
		Credits:      float64(creditSettings.RegistrationReward),
	...
	welcomeCfg := s.loadSMTPSettings(ctx)
	if welcomeCfg.Templates.WelcomeEnabled {
		go func(email, name string, cfg SMTPConfig) {
			_ = s.smtp.SendWelcome(context.Background(), cfg, email, name)
		}(created.Email, created.Name, welcomeCfg)
	}
...
func (s *AuthService) ResetPassword(ctx context.Context, email, password, emailCode, ip string) error {
	...
	user, err := s.users.SetPasswordByEmail(ctx, normalizedEmail, passwordHash)
	if err != nil {
		return err
	}
	return s.sessions.DestroyUser(ctx, user.ID)
}
...
func (s *AuthService) ChangePassword(ctx context.Context, userID, currentPassword, newPassword string) error {
	...
	return s.sessions.DestroyUser(ctx, userID)
}
...
func (s *AuthService) AuthConfig(ctx context.Context) (map[string]any, error) {
	...
	return map[string]any{
		...
		"registration_reward":   credits.RegistrationReward,
...
func (s *AuthService) loadSMTPSettings(ctx context.Context) SMTPConfig {
	...
	return SMTPConfig{
		...
		SiteName:      strings.TrimSpace(title),
		ExpireMinutes: parseIntSetting(ttlRaw, 600) / 60,
