...
type PaySettings struct {
	...
	Packages    []PayPackage `json:"packages"`
}
...
func defaultPayPackages() []PayPackage {
	return []PayPackage{
		{ID: "starter", Name: "入门包", Amount: 10, Points: 1000, Enabled: true, Sort: 1},
		{ID: "creator", Name: "创作者包", Amount: 50, Points: 5500, Enabled: true, Sort: 2},
		{ID: "studio", Name: "工作室包", Amount: 100, Points: 12000, Enabled: true, Sort: 3},
	}
}
...
func (s *PaymentService) Settings(ctx context.Context) PaySettings {
	...
	var packages []PayPackage
	if raw := strings.TrimSpace(s.get(ctx, "pay.packages")); raw != "" {
		_ = json.Unmarshal([]byte(raw), &packages)
	}
	if len(packages) == 0 {
		packages = defaultPayPackages()
	}
	return PaySettings{
...
func (s *PaymentService) SaveSettings(ctx context.Context, in PaySettings) error {
	...
	if len(in.Packages) == 0 {
		in.Packages = defaultPayPackages()
	}
	...
	for i := range in.Packages {
		...
		if p.ID == "" || p.Name == "" || p.Amount <= 0 || p.Points <= 0 || seen[p.ID] {
			return errors.New("充值套餐配置无效：ID、名称、金额和额度必须唯一且大于 0")
		}
		seen[p.ID] = true
		if p.Sort <= 0 {
			p.Sort = i + 1
	...
	packages, err := json.Marshal(in.Packages)
	if err != nil {
		return err
	}
...
func (s *PaymentService) CreateOrder(ctx context.Context, user *model.User, amount float64, method, packageID, notifyBase, clientIP string) (*model.Order, error) {
	cfg := s.Settings(ctx)
	if !cfg.Enabled {
	...
	if packageID != "" {
		for _, item := range cfg.Packages {
			if item.ID == packageID && item.Enabled {
				amount = item.Amount
				packagePoints = item.Points
...
func (s *PaymentService) Continue(ctx context.Context, user *model.User, orderID, notifyBase, clientIP string) (*model.Order, error) {
	...
	packageID := ""
	for _, item := range s.Settings(ctx).Packages {
		if item.Enabled && item.Name == old.Remark && item.Points == old.Points && int64(math.Round(item.Amount*100)) == int64(math.Round(old.Amount*100)) {
			packageID = item.ID
			break
		}
	}
	return s.CreateOrder(ctx, user, old.Amount, old.PayType, packageID, notifyBase, clientIP)
}
...
func (s *PaymentService) HandleNotify(ctx context.Context, params map[string]string) (bool, error) {
	...
	if strings.TrimSpace(params["pid"]) != cfg.PID {
		return false, errors.New("merchant id mismatch")
	}
	...
	notifyAmount, err := strconv.ParseFloat(strings.TrimSpace(params["money"]), 64)
	if err != nil || notifyAmount < 0 || math.IsNaN(notifyAmount) || math.IsInf(notifyAmount, 0) {
		return false, errors.New("invalid payment amount")
	}
	// Compare integer cents so harmless decimal formatting differences do not
	// reject a valid callback and floating-point rounding cannot accept a mismatch.
	if int64(math.Round(notifyAmount*100)) != int64(math.Round(order.Amount*100)) {
		return false, errors.New("payment amount mismatch")
	}
	return s.orders.MarkPaidAndCredit(ctx, order.ID, params["trade_no"], time.Now())
}
