...
func NewMaintenanceService(tokens *repo.TokenRepository, tokenSvc *TokenService, events *repo.EventRepository, users *repo.UserRepository, credits *repo.CreditRepository, refresh *RefreshProfileService, settings *repo.SiteSettingRepository, store *storage.Client, inflight *InflightRegistry, showcase *repo.ShowcaseRepository, orders *repo.OrderRepository) *MaintenanceService {
...
func (m *MaintenanceService) tick(ctx context.Context) {
	...
	if purged, err := m.events.PurgeStale(ctx, m.stalePending); err != nil {
		...
		for _, e := range purged {
			...
			if m.credits != nil && e.CreditTransactionID != "" {
				if _, ok, err := m.credits.Refund(ctx, e.CreditTransactionID); err != nil {
					log.Printf("maintenance: refund abandoned event %s: %v", e.ID, err)
					continue
				} else if ok {
					_, _ = m.events.MarkRefunded(ctx, e.ID)
					refunded++
				}
				continue
			}
