...
func (s *TokenService) checkPendingAdobe(tokenID, cookie string) {
	...
	if cb, e := s.adobe.FetchCreditsBalance(ctx, result.AccessToken); e == nil {
		quotaMeta["cached_quota_at"] = int(time.Now().Unix())
		quotaUnknown := boolValueWithDefault(cb["unknown"], false)
		if quotaUnknown {
			quotaMeta["quota_unknown"] = true
			quotaMeta["quota_error"] = strings.TrimSpace(stringValue(cb["error"]))
		}
	...
	// ExchangeCookie succeeded, so this is a valid credential. A credits endpoint
	// block/shape change must remain "quota unknown", not become a dead account;
	// the generation request remains the authoritative entitlement check.
	if !planKnown {
		log.Printf("token import: adobe %s quota/plan unavailable, keeping active", tokenID)
...
func (s *TokenService) finishPending(ctx context.Context, pool, id, status string, dead bool, quotaMeta map[string]any) {
	...
	patch := map[string]any{"status": status, "meta": meta, "dead": dead}
...
func (s *TokenService) Quota(ctx context.Context, pool, id string) (map[string]any, error) {
	...
	if poolToType(item.Pool) == "adobe" && s.adobe != nil {
		...
		patch := map[string]any{}
		// Retry identity hydration during every manual sync. Some Adobe access
		// tokens do not carry an email claim, and the first profile request made
		// immediately after import can fail transiently or be region-blocked.
		if profile, profileErr := s.adobe.FetchAccountProfile(ctx, item.Value); profileErr == nil {
		...
		quotaUnknown := boolValueWithDefault(data["unknown"], false) || plan == ""
		if quotaUnknown {
			meta["quota_unknown"] = true
			meta["quota_error"] = strings.TrimSpace(stringValue(data["error"]))
			// A non-auth credits failure says nothing about credential validity.
			// Restore rows previously disabled by the old plan-unknown policy.
			patch["status"] = "active"
			patch["dead"] = false
		} else {
		...
		if !quotaUnknown {
			isVIP := plan != "free"
			patch["concurrency"] = 1
			patch["image_limited"] = !isVIP
			patch["video_limited"] = !isVIP
			if isVIP {
				patch["concurrency"] = 5
			}
		}
...
func accountRow(item model.TokenAccount, inFlight int64) map[string]any {
	...
	return map[string]any{
		...
		"display_name":         emptyToNil(item.AccountDisplayName),
		"account_email":        emptyToNil(item.AccountEmail),
		"account_display_name": emptyToNil(item.AccountDisplayName),
		...
		"used":                 item.Meta["cached_quota_used"],
		"total":                item.Meta["cached_quota_total"],
		"quota_unknown":        boolValueWithDefault(item.Meta["quota_unknown"], false),
		"quota_error":          emptyToNil(strings.TrimSpace(stringValue(item.Meta["quota_error"]))),
