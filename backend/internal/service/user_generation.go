...
func (s *UserGenerationService) DeleteImageHistory(ctx context.Context, user *model.User, eventID string) error {
	if user == nil || strings.TrimSpace(user.ID) == "" {
		return errors.New("未登录或会话已过期")
	}
	event, err := s.events.GetByID(ctx, strings.TrimSpace(eventID))
	if err != nil {
		return err
	}
	if event == nil || event.UserID != user.ID || event.Kind != "image" || event.Source != "user" {
		return errors.New("图片记录不存在")
	}
	if event.Status != "success" && event.Status != "failed" {
		return errors.New("任务仍在处理中，暂时不能删除")
	}
	file := strings.TrimLeft(strings.TrimSpace(event.File), "/")
	if file != "" && !strings.HasPrefix(file, "http://") && !strings.HasPrefix(file, "https://") && s.v1.store != nil && s.v1.store.Configured() {
		if err := s.v1.store.Delete(ctx, file); err != nil {
			return err
		}
		_ = s.v1.store.Delete(ctx, ThumbKey(file))
	}
	return s.events.DeleteOwnedByID(ctx, event.ID, user.ID)
}
