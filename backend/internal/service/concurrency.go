...
// count can never leak forever. If Redis is unavailable it fails closed: losing
// concurrency coordination must not fan traffic into the same provider account.
...
// `token`. Returns true if admitted. Fail-closed when Redis is down/unset.
