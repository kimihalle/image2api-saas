...
var ErrAdminProtected = errors.New("管理员账号不能停用或删除")
...
func (s *AdminWriteService) UpdateUser(ctx context.Context, userID string, body map[string]any) (*model.User, error) {
	...
	if _, ok := body["role"]; ok {
		...
		if newRole != "admin" && current.Role != "admin" {
			patch["role"] = newRole
		}
	}
...
func (s *AdminWriteService) AdjustUserCredits(ctx context.Context, userID string, delta float64) (*model.User, error) {
	order := &model.Order{ID: "M" + time.Now().Format("20060102150405") + randomUpper(6), Remark: "管理员调整余额"}
	user, err := s.orders.AdjustCreditsAndRecord(ctx, userID, delta, nil, order)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
...
func (s *AdminWriteService) SetUserCredits(ctx context.Context, userID string, value float64) (*model.User, error) {
	...
	order := &model.Order{ID: "M" + time.Now().Format("20060102150405") + randomUpper(6), Remark: "管理员设置余额"}
	user, err := s.orders.AdjustCreditsAndRecord(ctx, userID, 0, &value, order)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrNotFound
		}
...
func (s *AdminWriteService) UpdateShowcase(ctx context.Context, entryID string, body map[string]any) (*model.ShowcaseItem, error) {
	...
	if _, ok := body["upstream_model"]; ok {
		patch["upstream_model"] = strings.TrimSpace(stringValue(body["upstream_model"]))
	}
	if _, ok := body["capabilities"]; ok {
		patch["capabilities"] = jsonMap(body["capabilities"])
	}
...
func (s *AdminWriteService) CreateModel(ctx context.Context, body map[string]any) (*model.ModelConfig, error) {
	...
	if modelType == "image" && strings.EqualFold(provider, "adobe") {
		prices = withoutPriceTier(prices, "4K")
		pricesAgent = withoutPriceTier(pricesAgent, "4K")
	}
	...
	item := &model.ModelConfig{
		...
		FreeAllowed:         boolValueWithDefault(body["free_allowed"], false),
...
func (s *AdminWriteService) UpdateModel(ctx context.Context, modelID string, body map[string]any) (*model.ModelConfig, error) {
	current, err := s.models.Get(ctx, modelID)
	if err != nil {
		return nil, err
	}
	patch := map[string]any{}
	...
	if raw, ok := body["free_allowed"]; ok && raw != nil {
	...
	if value, ok := patch["type"].(string); ok {
		effectiveType = value
	}
	effectiveProvider := current.Provider
	if value, ok := patch["provider"].(string); ok {
		effectiveProvider = value
	}
	if effectiveType == "image" && strings.EqualFold(effectiveProvider, "adobe") {
		prices := current.Prices
		if value, ok := patch["prices"].(datatypes.JSONMap); ok {
			prices = value
		}
		prices = withoutPriceTier(prices, "4K")
		pricesAgent := current.PricesAgent
		if value, ok := patch["prices_agent"].(datatypes.JSONMap); ok {
			pricesAgent = value
		}
		patch["prices"] = prices
		patch["prices_agent"] = withoutPriceTier(pricesAgent, "4K")
		patch["resolutions"] = resolutionsFromPrices(prices)
	}
	return s.models.Update(ctx, modelID, patch)
}

func withoutPriceTier(prices datatypes.JSONMap, tier string) datatypes.JSONMap {
	out := make(datatypes.JSONMap, len(prices))
