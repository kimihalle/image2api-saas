...
import { IconCheckCircle, IconDelete, IconExclamationCircle, IconFileImage as IconUpload, IconRefresh } from '@arco-design/web-vue/es/icon'
...
async function load() {
  ...
  const [site, reg, smtpResponse, templateResponse, creditResponse, deaiResponse, mediaResponse, logResponse, announcementResponse, proxyResponse, payResponse] = await Promise.all([
    api('/settings/site'), api('/settings/registration'), api('/settings/smtp'), api('/settings/smtp/templates'),
    api('/settings/credits'), api('/settings/deai'), api('/settings/media'), api('/settings/logs'), api('/settings/announcement'), api('/settings/proxy'), api('/settings/pay'),
  ])
    ...
    <a-spin :loading="loadingData" class="settings-body">
      <a-tabs v-model:active-key="activeTab" type="line">
        ...
        <a-tab-pane key="announcement" title="平台公告">
          <section><div class="section-copy"><h3>用户公告</h3><p>保存新内容后会向所有尚未阅读该版本的用户弹出。清空并保存可关闭公告。</p></div><div><a-textarea v-model="announcement.content" :max-length="5000" show-word-limit :auto-size="{ minRows: 10, maxRows: 18 }" placeholder="输入维护通知、活动规则或服务变更" /><div v-if="announcement.content.trim()" class="announcement-preview"><strong>用户端预览</strong><p>{{ announcement.content }}</p></div><div class="form-actions"><a-button type="primary" :loading="busy === 'announcement'" @click="saveAnnouncement">{{ announcement.content.trim() ? '发布公告' : '关闭公告' }}</a-button></div></div></section>
        </a-tab-pane>
